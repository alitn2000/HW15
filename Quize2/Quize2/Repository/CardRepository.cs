...
public class CardRepository : ICardRepository
{
    ...
    public bool LogIn(string userName,string cardNo, string pass)
    {
        return _context.Cards.Any(c =>c.HolderName ==userName && c.CardNumber == cardNo && c.Password ==pass);
    }
   ...
   public bool PlusMoney(string cartNo, float money)
    ...
    public List<Card>? GetUsersCards(string holderName)
    {
        return _context.Cards.Where(c => c.HolderName == holderName).ToList();
    }

    public bool UpdatePass(string cardNo,string oldPass, string newPass)
    {
        var  Card = GetCardByCardNo(cardNo);
